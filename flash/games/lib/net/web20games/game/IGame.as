package net.web20games.game {
	...
	public interface IGame {
		...
		/**
		 * Called to initialize a new game. The game should examine the container
		 * (baseConfig, extendedConfig...) to act properly.
		 *
		 * @param snapshot
		 * Non-null if this player enters the game room when a game is taking place.
		 * From this snapshot the game must recover the game.
		 *
		 * @return
		 * A_ANY               Any player can make the next move.
		 * Non-negative number Index of the player who should make the next move.
		 */
		function onNewGame(snapshot:Object):int;
